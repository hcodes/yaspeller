...
function makeLink(resource) {
    ...
    if(resource.search(/^https?:/) === -1) {
        res = 'file://' + (resource.search(/^\//) === -1 ? '' : '/') + resource;
    }
...
module.exports = {
    ...
    onend: function(data) {
        ...
        try {
            fs.writeFileSync('yaspeller_report.html', template.replace(/\{\{content\}\}/, html));
            console.log(chalk.cyan('HTML report: ./yaspeller_report.html'));
        } catch(e) {
            console.error(e);
        }
    }
