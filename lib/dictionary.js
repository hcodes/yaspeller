...
module.exports = {
    ...
    loadDictionary(file) {
        ...
        printDebugInfo('Get/check dictionary: ' + file);

        try {
            data = loadFileAsJson(file, true);

            printDebugInfo('Use dictionary: ' + file);
        } catch (e) {
    ...
    checkTyposInDictionary(words, file) {
        ...
        if (hasTypos) {
            ...
            typos.forEach(function(typo) {
                console.log(chalk.cyan(typo + ' - en: ' + replaceRusLettersWithAsterisk(typo) +
                    ', ru: ' + replaceEngLettersWithAsterisk(typo)));
            });
            console.log('');
        }
