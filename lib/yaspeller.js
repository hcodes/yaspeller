...
function getFileFormat(file, format) {
    ...
    return params.htmlExts.indexOf(getExtension(file)) > -1 ? 'html' : 'plain';
}
...
function getUrlFormat(url, format) {
    ...
    if(ext) {
        return params.htmlExts.indexOf(ext) > -1 ? 'html' : 'plain';
    }
 ...
 * @param {string|Array} [settings.lang] Language: ru, uk, en or tr
 ...
 * @param {Function} [callback] Callback on each url
 */
function checkSite(url, commonCallback, settings, callback) {
    // TODO
}
