...
const { printDebugInfo } = require('../lib/helpers/debug');
...
function checkText(originalText, callback, settings) {
    ...
    printDebugInfo(`Original text: ${originalText}`);
    ...
    printDebugInfo(`Prepared text for API: ${text}`);
    ...
    async.parallelLimit(tasks, getMaxRequest(apiSettings), function(err, data) {
        ...
        printDebugInfo('Yandex.Speller API response:');
        printDebugInfo(jsonStringify(buf));
...
function checkFile(file, callback, settings) {
    ...
    printDebugInfo('Get file: ' + file);

    if (fs.existsSync(file)) {
        if (fs.statSync(file).isFile()) {
            const buf = fs.readFileSync(file);
            if (isutf8(buf)) {
                printDebugInfo('Post text → Yandex.Speller API: ' + file);
...
function checkSitemap(url, commonCallback, settings, callback) {
    ...
    printDebugInfo('Get sitemap: ' + url);
